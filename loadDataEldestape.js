...
const PORTADA = 'https://www.eldestapeweb.com/';
...
async function getNews() {
   ...
   const assetUrls = await page.$$eval('article a', assetLinks => assetLinks.map(link => link.href).filter(link => link.indexOf('eldestapeweb') != -1 && link.indexOf('tag') == -1  && link.indexOf('seccion') == -1));
...
async function getNewInfo(urls){
    ...
    for (let url of urls) {
        ...
        const assetUrls = await page.evaluate(()=>{
            let todos = document.querySelectorAll("div.cuerpo p");
            var summary;
            try{ summary = document.querySelector('p.intro').textContent } catch (error){ summary = " "; };
            let title;
...
async function writeJson(yourArray){
    ...
    fs.writeFile('./data/dataEldestape.json', myJsonString , function(err) {
...
async function writeArrayFile(first10url) {
    var file = fs.createWriteStream('urlsEldestape.txt');
