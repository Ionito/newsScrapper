...
async function getNewInfo(urls){
    ...
    for (let url of urls) {
        ...
        const assetUrls = await page.evaluate(()=>{
            ...
            var summary;
            try{ summary = document.querySelector('.articulo-subtitulos').textContent } catch (error){ summary = " "; };
            let title;
            try{ title = document.querySelector('.articulo-titulo').textContent; } catch (error) { title = " " };
            let body; 
