...
const PORTADA = 'https://www.tiempoar.com.ar/';
...
async function getNews() {
   ...
   const assetUrls = await page.$$eval('div.card a', assetLinks => assetLinks.map(link => link.href).filter(link => link.indexOf('tiempoar') != -1 )); //&& link.indexOf('autor') == -1 && link.indexOf('columnistas') == -1 && link.indexOf('seccion') == -1 
...
async function getNewInfo(urls){
    ...
    for (let url of urls) {
        ...
        const assetUrls = await page.evaluate(()=>{
            ...
            var summary;
            try{ summary = document.querySelector('div.art-lead p').textContent } catch (error){ summary = " "; };
            let title;
            try{ title = document.querySelector('.art-headline').textContent; } catch (error) { title = " " };
            let body; 
...
async function writeJson(yourArray){
    ...
    fs.writeFile('./data/dataTiempoAr.json', myJsonString , function(err) {
...
async function writeArrayFile(first10url) {
    var file = fs.createWriteStream('urlsTiempoAr.txt');
